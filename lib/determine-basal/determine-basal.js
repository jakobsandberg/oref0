...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data) {
    ...
    rT.IOB=iob_data.iob;
    if (iob_data.iob > 0) {
        var iobFactor = 1 + (iob_data.iob/10);
        target_bg = Math.floor(iobFactor * target_bg);
    }
    ...
    if (iob_data.iob > max_iob) {
        ...
        // rate required to deliver insulinReq more insulin over 10m:
	var idealSmb = Math.floor(Math.min(insulinReq / 2, 1)*10)/10;
	var durationInMinutes = 10;
	var idealSmbAsBasal = idealSmb * (60/durationInMinutes);
	var rate = basal + idealSmbAsBasal;
	rate = round_basal(rate, profile);
	var smbAsBasal = {
		time: (new Date()).getTime(),
		durationInMinutes: durationInMinutes,
		idealSmb: idealSmb,
		idealSmbAsBasal: idealSmbAsBasal,
		rate: rate
	};
	if (idealSmbAsBasal > 0) {
	  console.error('Simulating SMB as large basal:', JSON.stringify(smbAsBasal));
	}
